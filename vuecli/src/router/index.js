...
newRouter.beforeEach((to, from, next) => {
  to.fullPath == '/layout/vueRouteLife' ? console.log('全局守卫beforeEach', to, from) : ''
  next()
})
// 全局解析守卫：在导航被确认之前，同时在所有路由独享守卫和异步路由组件被解析之后，解析守卫就被调用
newRouter.beforeResolve((to, from, next) => {
  to.fullPath == '/layout/vueRouteLife' ? console.log('全局解析守卫beforeResolve', to, from) : ''
  next()
})
// 全局后置守卫：在导航被确认之前，同时在所有组件内守卫和异步路由组件被解析之后，解析守卫就被调用
newRouter.afterEach((to, from) => {
  to.fullPath == '/layout/vueRouteLife' ? console.log('全局后置守卫afterEach', to, from) : ''
})

console.log('_+_+_+_+router.js打印:', newRouter);
